...
function loadCategories() {
  fetch("data/categories.json")
    ...
    .catch(err => {
      document.getElementById("content").innerHTML = `<div class="alert alert-danger">Error loading categories: ${err.message}</div>`;
    });
}
...
function loadCategory(shortname) {
  fetch(`data/${shortname}.json`)
        ...
        content.innerHTML += `
          <div class="col-md-6 mb-4">
            <div class="card h-100 text-center">
              <img src="${getPlaceholderUrl(item.name)}" class="card-img-top" alt="${item.name}">
              <div class="card-body">
    ...
    .catch(err => {
      document.getElementById("content").innerHTML = `<div class="alert alert-danger">Error loading category: ${err.message}</div>`;
    });
}
...
function loadRandomCategory() {
  fetch("data/categories.json")
    ...
    .catch(err => {
      document.getElementById("content").innerHTML = `<div class="alert alert-danger">Error loading random category: ${err.message}</div>`;
    });
}
